<template>
  <div class="main">
    <button class="re" @click="re()"></button>
    <div class="box">
      <span class="dati">无尽排位</span>
      <div class="question" v-if="startflag">
        <span>{{ question.question }}</span>
      </div>
      <div class="option" v-if="startflag">
        <el-radio-group v-model="radio">
          <el-radio :label="1" @change="submit(1)">{{
            question.answer1
          }}</el-radio>
          <el-radio :label="2" @change="submit(2)">{{
            question.answer2
          }}</el-radio>
          <el-radio :label="3" @change="submit(3)">{{
            question.answer3
          }}</el-radio>
          <el-radio :label="4" @change="submit(4)">{{
            question.answer4
          }}</el-radio>
        </el-radio-group>
      </div>

      <div id="startGame" v-if="!startflag">
        <span @click="StartGame">开始答题</span>
      </div>
    </div>
    <div class="mask" :style="'display:' + showflag" v-if="showModal"></div>
    <div class="mask_box" v-if="showModal">
      <span>{{
        questionflag ? "恭喜你回答正确" : "回答错误，请继续努力"
      }}</span>
      <span style="top: 286px">获得积分:{{ questionflag ? "2" : "0" }}</span>
      <button class="determine" @click="nextQ">下一题</button>
    </div>
  </div>
</template>
